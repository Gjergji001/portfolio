...
export class MenuComponent {
  ...
  private updateBubbles(t: number): void {
    if (!this.mouseMoved) {
      this.mouse.tX = (0.5 + 0.3 * Math.cos(0.002 * t) * (Math.sin(0.005 * t))) * window.innerWidth;
      this.mouse.tY = (0.5 + 0.2 * (Math.cos(0.005 * t)) + 0.1 * Math.cos(0.01 * t)) * window.innerHeight;
    }
    ...
    if (this.ctx) {
      ...
      for (let i = 1; i < this.touchTrail.length - 1; i++) {
        ...
        this.ctx.quadraticCurveTo(this.touchTrail[i].x, this.touchTrail[i].y, xc, yc);
        this.ctx.lineWidth = this.params.widthFactor * (this.params.pointsNumber - i);
        this.ctx.stroke();
      }

      this.ctx.lineTo(this.touchTrail[this.touchTrail.length - 1].x, this.touchTrail[this.touchTrail.length - 1].y);
      this.ctx.stroke();

      this.mouse.x += (this.mouse.tX - this.mouse.x) * this.params.mouseThreshold;
      this.mouse.y += (this.mouse.tY - this.mouse.y) * this.params.mouseThreshold;
